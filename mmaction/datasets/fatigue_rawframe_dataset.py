...
class FatigueCleanDataset(BaseDataset):
    ...
    def load_annotations(self):
        video_infos = []
        flatten_video_infos = []
        label_map = {'fatigue_close': 1, 'fatigue_dahaqian':1, 'fatigue_look_down': 0, 'fatigue_squint':0, 'smoking':0, 'calling':0}
        print("Start to Parsing label file ", self.ann_file)
...
@DATASETS.register_module()
class FatigueCleanDatasetWithoutDahaqian(BaseDataset):
    """Rawframe dataset for action recognition.
    ...
    def load_annotations(self):
        ...
        with open(self.ann_file, 'r') as fp:
            ...
            for vname in anns:
                ...
                # video label
                if vinfo['label'] not in label_map:
                    continue
                fat_label = label_map[vinfo['label']]
...
@DATASETS.register_module()
class FatigueMoveDataset(BaseDataset):
    """Rawframe dataset for action recognition.
    ...
    def get_valid_fatigue_start_idx(self, rect_infos, min_frames_before_fatigue, fatigue_idxs, video_dir, max_frames=500):
        global no_facerect_count
        global no_file_list
        ...
        valid_idxs = []
        for fat_end_idx in fatigue_idxs:
            # Let Fatigue warning index in the middle
            if fat_end_idx - int(min_frames_before_fatigue/2) <= 0:
                fat_start_idx = max(fat_end_idx - int(min_frames_before_fatigue/2), 1)
                fat_end_idx = min(fat_start_idx + min_frames_before_fatigue - 1, max_id)
            else:
                fat_end_idx = min(fat_end_idx + int(min_frames_before_fatigue/2), max_id)
                fat_start_idx = max(fat_end_idx - min_frames_before_fatigue + 1, 1)
            # random moving window, optional,todo
            # border control
            if idx_rect_map[fat_start_idx:fat_end_idx + 1].sum() == min_frames_before_fatigue:
                # valid idx
                valid_idxs.append(fat_end_idx)

        # merge fatigue end index, optional
    ...
    def load_annotations(self):
        ...
        with open(self.ann_file, 'r') as fp:
            ...
            for vname in anns:
                ...
                # get valid fatigue index according to facerect infos and fatigue index
                fat_idxs = self.get_valid_fatigue_start_idx(rect_infos, self.min_frames_before_fatigue, fatigue_idxs, video_path, max_frames=total_frames)
                if len(fat_idxs) < 1:
                    statistics_info['invalid'] += 1
                    statistics_info[fat_label]['invalid'] += 1
                    continue
